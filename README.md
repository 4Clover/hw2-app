...
The included `Dockerfile` uses a multi-stage build to create an optimized image containing both the built frontend and the backend.

1.  **Build the Docker Image:**
    *(Make sure Docker Desktop or Docker Engine is running)*
    ```bash
    # Run from the project root directory (where the Dockerfile is)
    docker build -t [your-app-name] .
    ```
    Replace `[your-app-name]` with a suitable tag (e.g., `hw2-app`).

2.  **Run the Docker Container:**
    ```bash
    docker run -p 8000:8000 --name [your-container-name] [your-app-name]
    ```
    *   `-p 8000:8000`: Maps port 8000 on your host machine to port 8000 inside the container (where Flask is running).
    *   `--name [your-container-name]`: Assigns a memorable name to the running container (optional).
    *   `[your-app-name]`: The tag you used when building the image.
